...
import { fileNameFromPath, truncateFileName, detectScenes, DetectionSettings, DEFAULT_DETECTION_SETTINGS } from "../utils/episodeUtils";
...
export default function useImportExport(props: ImportExportProps) {
  ...
  const [detectionSettings, setDetectionSettings] = useState<DetectionSettings>(DEFAULT_DETECTION_SETTINGS); // [AMVerge Plus]
  ...
  const handleImport = async (file: string | null) => {
    ...
    try {
      ...
      const formatted = await detectScenes(file, episodeId, detectionSettings);
  ...
  const handleExport = async(selectedClips: Set<string>, mergeEnabled: boolean, mergeFileName?: string, remuxEnabled?: boolean) => { // [AMVerge Plus] remuxEnabled param
    ...
    try {
        ...
        const videoEncoder = localStorage.getItem("amverge_encoder_v1") || "libx264"; // [AMVerge Plus]

        await invoke("export_clips", {
            clips: clipArray,
            savePath: savePath,
            mergeEnabled: true,
            remuxEnabled: remuxEnabled ?? false, // [AMVerge Plus]
            videoEncoder, // [AMVerge Plus]
        });
        } else {
        ...
        const videoEncoder = localStorage.getItem("amverge_encoder_v1") || "libx264";

        await invoke("export_clips", {
            clips: clipArray,
            savePath: savePath,
            mergeEnabled: false,
            remuxEnabled: remuxEnabled ?? false,
            videoEncoder,
        });
        }
