...
export default function PreviewContainer (props: PreviewContainerProps) {
  ...
  const onExportClick = () => {
    if (mergeEnabled) {
      ...
      props.handleExport(props.selectedClips, false, undefined, remuxEnabled);
    }
  };
        ...
        <div className="checkbox-row">
          <label className="custom-checkbox">
            <input
              type="checkbox"
              className="checkbox"
              checked={remuxEnabled}
              onChange={(e) => setRemuxEnabled(e.target.checked)}
            />
            <span className="checkmark"></span>
          </label>
          <p title="Stream copy only — no re-encode. Fastest, zero quality loss. May not be compatible with all NLEs.">Remux (no re-encode)</p>
        </div>
